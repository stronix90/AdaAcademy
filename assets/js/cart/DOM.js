function DOM_refreshCartParameters() {
    ...
    DOM_Cart_Parameters_Amount.html(convCurrency(cartTotalAmount))
    ...
    if (cartCoursesCount > 0) DOM_Cart_Button_Counter.html(cartCoursesCount).show(500);
...
function DOM_Cart_AddItem(id) {
    ...
    <li id="cart_item_${id}" class="list-group-item">
        ${courses[id].name} - 
        <span class="curr_Simbol">${CURRENCY.Symbol}</span>
        <span class="price" data-price="${courses[id].price}">${convCurrency(courses[id].price)}</span> -
